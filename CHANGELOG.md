...
- **Four text formats: executive summary, FAQ, social posts and newsletter text** (NEXT-182). Each is a checkbox on the job form (on by default, like the media artifacts) and one schema-validated nr-llm call (`completeStructured()`, one repair round-trip on a mismatch), written in the source language and steered by the *audience* and *tone of voice* snippets. The executive summary has at most eight sentences, key facts first. The FAQ keeps at most ten question/answer pairs taken only from the source and adds a schema.org `FAQPage` JSON-LD block to paste into a page. The social posts are one artifact per platform — LinkedIn ≤ 3000, X ≤ 280, Instagram ≤ 2200 characters including hashtags — and the limits are enforced in code by cutting at the last sentence end that fits, not only asked for in the prompt. The newsletter has a subject, a preheader, plain paragraphs and exactly one call to action. The text is stored on the artifact (`script_text`, structured answer in `metadata.content`); no file is written. An unusable answer fails only that artifact, with the reason. The text formats make no speech or image call and need neither `generate_audio` nor `generate_vision`. Four `want_*` columns are added to `tx_nrrepurpose_domain_model_job` — run the database analyzer after the update. See ADR-004.
